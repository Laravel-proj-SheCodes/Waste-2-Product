        ...
        @else
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                @foreach ($posts as $post)
                    <div class="troc-card">
                        @php
                            $photoPaths = $post->photos ?? []; // Utilise le cast automatique, gère les cas null
                        @endphp
                        @if (!empty($photoPaths))
                        ...
                        <div class="mt-3 space-x-1">
                            ...
                            <a href="{{ route('offres-troc.create', $post->id) }}" class="troc-btn troc-btn-propose">Proposer une Offre</a>
                            <a href="{{ route('postdechets.offres', $post->id) }}" class="troc-btn" style="background: linear-gradient(45deg, #9b59b6, #8e44ad); border: 2px solid #9b59b6; box-shadow: 0 3px 10px rgba(155, 89, 182, 0.3);">Voir Offres</a>
                        </div>
                    </div>
                @endforeach
            </div>
