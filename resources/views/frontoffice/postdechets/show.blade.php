...
@php
    ...
    if (count($photos)) {
        ...
        if (Storage::disk('public')->exists($first))      $main = Storage::disk('public')->url($first);
        elseif (file_exists(public_path('storage/'.$first))) $main = asset('storage/'.$first);
    }

    $received = isset($received) ? $received : collect();

    // Droit d’afficher le bouton chat
    $postDechet->loadMissing('propositions');
    $acceptedStatuses = ['accepte','accepted','acceptee','acceptée'];
    $uid     = auth()->id();
    $isOwner = $uid && ($postDechet->user_id === $uid);

    $acceptedForOwner = $isOwner
        ? $postDechet->propositions->first(fn($p) => in_array(strtolower((string)($p->statut ?? '')), $acceptedStatuses, true))
        : null;

    $acceptedForMe = (!$isOwner && $uid)
        ? $postDechet->propositions->first(fn($p) =>
...
@section('content')
<style>
  .hero{background:#198754;color:#fff;border-radius:16px;padding:24px 20px;margin-bottom:24px}
  ...
  .glow-target{animation:glowPulse 1s ease-in-out 5 alternate;box-shadow:0 0 0 rgba(255,193,7,0)}
  @keyframes glowPulse{0%{box-shadow:0 0 0 rgba(255,193,7,0)}50%{box-shadow:0 0 18px rgba(255,193,7,.8);background:rgba(255,243,205,.6)}100%{box-shadow:0 0 0 rgba(255,193,7,0)}}
  #chatBox .bubble{display:inline-block;padding:.45rem .65rem;border-radius:.6rem;max-width:80%;word-break:break-word}
...
<div class="container py-4">
  ...
  @if(request()->boolean('accepted'))
    <div class="alert alert-success mt-3">✅ Félicitations ! Votre proposition a été <strong>acceptée</strong>.</div>
  @endif
  @if(request()->boolean('rejected'))
    <div class="alert alert-danger mt-3">❌ Votre proposition a été <strong>refusée</strong>.</div>
  @endif

  <div class="hero mt-3">
    <div class="row align-items-center">
      <div class="col-md-8">
        <h1 class="h3 m-0">{{ $postDechet->titre }}</h1>
        <p class="mb-0">Catégorie <strong>{{ $postDechet->categorie }}</strong> •
    ...
    <div class="col-lg-5">
      <div class="card-soft p-4">
        ...
        @auth
          ...
          @else
            ...
            @if (!$dejaAcceptee && !$jAiDejaPropose)
              @if (Route::has('front.propositions.create') && $postDechet->type !== 'troc')
                <a href="{{ route('front.propositions.create', $postDechet) }}" class="btn btn-success w-100 mt-2">
                  Faire une proposition
                </a>
              @elseif (Route::has('offres-troc.create.front') && $postDechet->type === 'troc')
                <a href="{{ route('offres-troc.create.front', $postDechet) }}" class="btn btn-success w-100 mt-2">
          ...
          @if($acceptedProp)
            <button type="button" class="btn btn-success w-100 mt-3 open-chat"
                    data-proposition-id="{{ $acceptedProp->id }}">💬 Chat</button>
          @endif
        @else
          <a href="{{ route('login') }}" class="btn btn-outline-success w-100 mt-2">Connectez-vous pour proposer</a>
        @endauth
      </div>
    </div>
  </div>
        ...
        @else
          <div class="list-group">
            @foreach($received as $prop)
              @php
                $isAccepted = in_array(strtolower((string)($prop->statut ?? '')), ['accepte','accepted','acceptee','acceptée'], true);
                $badgeClass = $isAccepted ? 'bg-success' : (($prop->statut === 'refuse') ? 'bg-danger' : 'bg-secondary');
                $labels = ['en_attente'=>'en attente','accepte'=>'acceptée','accepted'=>'acceptée','acceptee'=>'acceptée','acceptée'=>'acceptée','refuse'=>'refusée'];
                $label = $labels[strtolower((string)($prop->statut ?? 'en_attente'))] ?? ($prop->statut ?? '—');
              @endphp
              ...
              <div id="proposition-{{ $prop->id }}" class="list-group-item">
                <div class="d-flex justify-content-between align-items-start">
                  <div class="me-3">
                    ...
                    <div class="mt-2">{!! nl2br(e($prop->description)) !!}</div>
                  </div>

                  <div class="text-end">
                    @if(($prop->statut ?? 'en_attente') === 'en_attente')
                      <form action="{{ route('front.propositions.accept', $prop) }}" method="POST" class="d-inline">
                        @csrf <button type="submit" class="btn btn-sm btn-success">Accepter</button>
                      </form>
                      <form action="{{ route('front.propositions.reject', $prop) }}" method="POST" class="d-inline">
                        @csrf <button type="submit" class="btn btn-sm btn-outline-danger">Refuser</button>
                      </form>
                    @endif

                    @if($isAccepted)
                      <button type="button" class="btn btn-sm btn-outline-success ms-1 open-chat"
                              data-proposition-id="{{ $prop->id }}">💬 Chat</button>
                    @endif
                  </div>
                </div>
              </div>
...
(function () {
  ...
  async function openChat(propositionId){
    try{
      const r = await fetch(`/chat/open/${propositionId}`, { method:'POST', headers:{ 'X-CSRF-TOKEN': csrf } });
      ...
      startPolling();
    }catch(e){ console.error(e); alert('Erreur réseau à l’ouverture du chat.'); }
  }
